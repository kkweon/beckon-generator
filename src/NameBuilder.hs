...
isModuleName xs = length splitText >= 2 && isBeckonModule (getPossibleModuleName splitText)
  where
    splitText :: [T.Text]
...
getSrcFilePath moduleName =
  ...
  where
    someName :: F.FilePath
    someName = T.unpack $ T.replace "." "/" (stripBeckonFromModuleName moduleName)

    componentName :: F.FilePath
    componentName = T.unpack $ getComponentName (stripBeckonFromModuleName moduleName)
